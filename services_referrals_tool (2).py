...
with hdr_c:
    ...
    st.markdown(
        f"<p style='text-align:center; font-weight:600; color:#C00000; margin-top:0;'>(as of {now_ct})</p>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align:center; font-size:16px; margin-top:0;'>"
        "Upload the <strong>10433</strong> Services/Referrals report (Excel). "
        "The tool will build: <em>Services & Referrals</em>, <em>PIR Summary</em>, "
        "<em>PIR Dashboard</em>, and <em>PIS Dashboard</em>."
        "</p>",
        unsafe_allow_html=True,
    )
st.divider()
...
def build_workbook(df_raw: pd.DataFrame, cutoff_value, require_pir: bool = True) -> bytes:
    df = df_raw.copy()
    ...
    PID_COL = next(
        (
            ...
            if "participant pid" in c.lower() or ("pid" in c.lower() and "participant" in c.lower())
        ),
        "ST: Participant PID",
    )
    ...
    AUTH_COL = next(
        (
            c for c in df.columns
            if ("author" in c.lower() and "service" in c.lower()) or "worker" in c.lower() or "staff" in c.lower()
        ),
        None,
    )
    ...
    df = df[df[DATE_COL].notna() & (df[DATE_COL] >= pd.Timestamp(cutoff_value))].copy()
    ...
    if AUTH_COL:
        cols.append(AUTH_COL)
    cols += [RES_COL, "Counts for PIR"]

    details = df[cols].copy()
    rename_map = {
        c: _clean_header(c)
        for c in details.columns
        if c not in ["Counts for PIR"]
    }
    ...
    per_child = (
        pir_rows.dropna(subset=["_pid_norm"])
        .drop_duplicates(subset=["_pid_norm", GEN_COL, "_PIR_CODE"])
        .groupby([GEN_COL, DET_COL])
        .size()
    ...
    per_family = (
        pir_rows.drop_duplicates(subset=["Family ID", GEN_COL, "_PIR_CODE"])
        .groupby([GEN_COL, DET_COL])
        .size()
    ...
    summary = summary[
        ["GENERAL service", "DETAILED services", "Distinct Children (PID)", "PIR (Distinct Families)"]
    ]
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        ...
        def _set_widths(ws, cols_to_size):
            for idx, name in enumerate(cols_to_size):
                n = str(name).lower()
                w = 20
                if "center" in n or "campus" in n:
                    w = 26
                if "general service" in n:
                    w = 30
                if "detail service" in n or "detailed service" in n:
                    w = 40
                if "date" in n:
                    w = 14
                if "result" in n:
                ...
                if "author" in n:
                    w = 24
                if "pid" in n or "family id" in n:
                    w = 16
                if "last name" in n or "first name" in n:
                    w = 22
                if "counts for pir" in n:
                    w = 18
        ...
        ws1.write_rich_string(
            1, 1,
            subtitle_fmt, "Services & Referrals — 2025-2026 as of ",
            red_fmt, f"({now_ct})",
            subtitle_fmt
        )
        ...
        for name in details.columns:
            if (
                "general service" in name.lower()
                or "detail service" in name.lower()
                or "author" in name.lower()
                or "center" in name.lower()
                or "campus" in name.lower()
            ):
                ws1.conditional_format(
                    4, name_to_idx[name], last_row_0, name_to_idx[name],
        ...
        ws4.merge_range(2, 1, 3, 2, "PIR Children", kpi_lbl)
        ws4.merge_range(4, 1, 5, 2, k_children, kpi_val)
        ws4.merge_range(2, 3, 3, 4, "PIR Families", kpi_lbl)
        ws4.merge_range(4, 3, 5, 4, k_families, kpi_val)
        ws4.merge_range(2, 5, 3, 6, "Detailed Rows", kpi_lbl)
        ws4.merge_range(4, 5, 5, 6, k_details, kpi_val)
        ...
        ws4.add_table(
            ...
            start_c,
            end_r,
            start_c + len(top_det.columns) - 1,
            {
                "name": "tblPIRDet",
                "columns": [{"header": h} for h in top_det.columns],
        ...
        ws5.add_table(
            ...
            gs_c,
            gs_end_r,
            gs_c + len(gen_month_sorted.columns) - 1,
            {
                "name": "tblGenMonth",
                "columns": [{"header": h} for h in gen_month_sorted.columns],
                "style": "Table Style Medium 2",
            },
        )
        ...
        ws5.add_table(
            ...
            rc_c,
            rc_end_r,
            rc_c + len(res_counts.columns) - 1,
            {
                "name": "tblResult",
                "columns": [{"header": h} for h in res_counts.columns],
                "style": "Table Style Medium 2",
            },
        )
        ...
        points = [
            {
                "data_labels": {
                    ...
                    "leader_lines": True,
                    "font": {"bold": True, "size": 14, "color": "black"},
                }
            }
        ...
        ws5.add_table(
            ...
            m_c,
            m_end_r,
            m_c + len(monthly_sorted.columns) - 1,
            {
                "name": "tblMonthly",
                "columns": [{"header": h} for h in monthly_sorted.columns],
                "style": "Table Style Medium 2",
            },
        )
