...
def build_workbook(df_raw: pd.DataFrame, cutoff: str, require_pir: bool = True) -> bytes:
    ...
    # Column ids (flexible to header text)
    FID_COL   = "Family ID"
    ...
    # Reason (exclude "Missing PIR code..." per your rule)
    def reason_fn(row):
    ...
    # ---- Sheet 2: PIR Summary (strict PIR)
    ...
    per_family = (pir_rows.drop_duplicates(subset=["Family ID", GEN_COL, "_PIR_CODE"])
                           .groupby([GEN_COL, DET_COL]).size()
    ...
    # ---- Sheet 3: Author Fix List (actionable only)
